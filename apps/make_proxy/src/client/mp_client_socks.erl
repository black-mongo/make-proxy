...
-export([detect_head/1, request/2]).
...
request(Data, #client{key = _Key, remote = Remote} = State) ->
    ok = gen_tcp:send(Remote, Data),
    {ok, State}.

%% http://www.openssh.com/txt/socks4.protocol
-spec find_target(binary()) ->
                     {ok, mp_target(), binary(), binary()} | {error, term()} | more.
...
-spec split_socks5_data(integer(), binary()) ->
                           {ok, mp_target(), binary()} | {error, term()} | more.
...
-spec split_socks4_data(binary()) -> {ok, string(), binary()} | {error, term()} | more.
...
split_socks4_data(<<0:8, Body/binary>>, UserID) ->
    ID = lists:flatten(
             lists:join("", lists:reverse(UserID))),
    {ok, ID, Body};
